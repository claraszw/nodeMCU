...
function collectData()
	...
	if send then
		...
		presenceValue = gpio.read(PRESENCE_PIN)

		print('Presence: ' .. presenceValue)

		if(presenceValue == gpio.HIGH) then
			presenceValue = true
		else
			presenceValue = false
		end
		...
		message = { source = NODE_ID, info = {luminosity = luminosityValue, temperature = temperatureValue, presence = presenceValue} }
